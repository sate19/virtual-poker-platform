...
| **亮一张** | 👁️ | 赢家被系统随机强制翻开一张手牌；每局结束后所有玩家均可逐张选择性亮牌 |
...
- 📢 **宣言令**：每过 N 局随机触发，翻牌前所有人必须展示一张手牌，否则强制弃牌
...
- 逐张亮牌：牌局结束后，每位玩家可以点击任意一张手牌向全桌展示，也可以一键展示全部。观战者只能看到已被亮出的牌。Show One 玩法触发时系统随机自动翻开赢家一张牌。
- 连胜追踪：服务端维护每位玩家的连胜计数，推送至前端展示 `🔥 ×N` 徽章，达到 3 连胜自动发放三连冠赏金。
- 小玩法事件通知：每次小玩法触发时，服务端广播 `game:event`，前端显示彩色动画横幅提示。
...
### 2026-06-11 — 亮牌系统 & 小玩法特效

- 重写亮牌系统：从"全亮/全隐"升级为**逐张亮牌**
- 牌局结束后，本人每张手牌旁显示紫色 "Show" 按钮，可点击单独展示一张或全部
- 其他人只能看到已被亮出的牌，未被亮的显示为 `?` 卡背
- **Show One**：系统随机翻开赢家一张牌，玩家可继续手动亮出另一张
- 新增小玩法特效横幅：炸弹底池（紫红）、7-2 赏金（橙红）、三连冠（金光）、亮一张（蓝紫）
- 连胜玩家座位上显示 `🔥 ×N` 脉冲辉光徽章
- 徽章和横幅带有入场滑动 + 辉光呼吸动画
- 预留玩法：📢 **宣言令**（Declare or Die）
